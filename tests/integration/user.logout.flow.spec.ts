...
describe('👤 User Logout integration flow', () => {
	...
	it('logs out successfully; toast shown; user cleared; navigates home; modal closes', async () => {
		...
		await (app.vm as any).handleLogout()
		await flushPromises()
		await nextTick()
	...
	it('logout server error (500); error toast; user unchanged; stays on route; modal closes', async () => {
		mswServer.use(
	...
	it('guard prevents logout when already loading; modal stays open', async () => {
		...
		;(app.vm as any).handleOpenModal(true)
		await nextTick()
		;(app.vm as any).isLoadingLogout = true
		await nextTick()
