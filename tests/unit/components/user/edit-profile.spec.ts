...
describe('components/User/EditProfile.vue', () => {
	...
	it('inline @keypress calls preventSpace for username and email only (positive)', async () => {
		...
		const usernameCmp = wrapper
			.findAllComponents(UInputStub)
			.find((c) => c.props('placeholder') === 'Username')!
		const emailCmp = wrapper
			.findAllComponents(UInputStub)
			.find((c) => c.props('placeholder') === 'Email address')!
